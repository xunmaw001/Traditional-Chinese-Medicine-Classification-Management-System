...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
                                                                              ...
                                                                              this.leixingOptions = "短缺,超储,过期".split(',')
                                              },
                                        // 多级联动参数
                                                    info(id) {
      this.$http({
        url: `yichangbaogaodan/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `yichangbaogaodan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yichangbaogaodanCrossAddOrUpdateFlag = false;
                  this.parent.search();
